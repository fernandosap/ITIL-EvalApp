...
// Startup-only schema bootstrap. Runtime request handlers intentionally do not
// call this function: deployments establish/validate shared tables before the
// HTTP server starts, so the first user never becomes a migration trigger.
...
// Cross-instance row lock. The returned release() holds the transaction open
// until the response finishes, serializing content mutations across CF nodes.
async function acquireRowLock(lockName, env = process.env) {
  ...
  return async function release(ok = true) {
    ...
    released = true;
    try {
      if (ok) await commit(conn); else await rollback(conn);
    } finally {
      try { conn.disconnect(); } catch (_e) { /* ignore */ }
    }
  };
