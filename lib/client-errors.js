...
//   - accessCode is validated against the exact 6-char format
//     or set to null.
...
//   format. The leading class includes URL/query separators
//   (=, ?, &) so codes embedded in URLs (e.g. `?code=ABC2DE`)
//   are caught. The word boundary prevents eating the
//   middle of longer words.
...
function sanitizeAccessCode(s) {
  if (typeof s !== 'string') return null;
  const m = s.match(/^[A-HJ-KM-NP-Z2-9]{6}$/);
  return m ? s : null;
}
...
function sanitizePayload(input) {
  ...
  const accessCode = sanitizeAccessCode(input.accessCode);
  return {
    type,
    screen,
    accessCode,                         // null or a validated 6-char code
