...
// Explicit application installer. server.js owns the Express instance and
// calls install(app) before registering routes; no require-cache or Express
// module monkey-patching is used.
...
async function questionMutationGuard(_req, res, next) {
  let release;
  try {
    release = await acquireRowLock('question_sets');
  } catch (err) {
    return res.status(503).json({ error: 'question_set_lock_unavailable', message: err.message });
  }
  let released = false;
