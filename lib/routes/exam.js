...
const { findRoute, prepend, replaceAfterFirst } = require('./helpers.js');
...
function migrate(app) {
  ...
  const originalProgressHandler = progress.route.stack[progress.route.stack.length - 1].handle;
  replaceAfterFirst(progress, 'post', '/api/progress', [proctor.progressIncidentMiddleware, originalProgressHandler]);
}
