...
function resolveAttemptSnapshot(row) {
  ...
  if (status === 'unused') {
    ...
    return {
      ...
      questionSetName: normalizeExamTitle(row.ASSIGNED_QUESTION_SET_NAME || ''),
  ...
  const name = normalizeExamTitle(rawName || '');
...
async function freezeAttemptSnapshot(code, questionSetId, questionSetName) {
  ...
  return withConnection(async (conn) => {
    ...
    const set = setRows[0] || {};
    const name = normalizeExamTitle(questionSetName || set.NAME || '') || null;
    const version = Number(set.VERSION_NUMBER);
    await exec(conn,
      `UPDATE ACCESS_CODES
          SET ATTEMPT_QUESTION_SET_ID = COALESCE(ATTEMPT_QUESTION_SET_ID, ?),
              ATTEMPT_QUESTION_SET_NAME = COALESCE(ATTEMPT_QUESTION_SET_NAME, ?),
              ATTEMPT_QUESTION_SET_VERSION = COALESCE(ATTEMPT_QUESTION_SET_VERSION, ?),
...
function freezeAttemptOnStart(req, res, next) {
  ...
  res.json = function snapshotStartJson(body) {
    ...
    return freezeAttemptSnapshot(req.body?.code, body.questionSet.id, body.questionSet.name)
      ...
      .catch((err) => res.status(500) && originalJson({ error: 'attempt_snapshot_failed', message: err.message }));
  };
  return next();
}
...
function clearSnapshotOnReset(req, res, next) {
  ...
  res.json = function resetSnapshotJson(body) {
    ...
    return clearAttemptSnapshot(req.body?.code)
      .then(() => originalJson(body))
      .catch((err) => originalJson({ ...body, warning: `attempt_snapshot_reset_failed:${err.message}` }));
  };
  return next();
}
...
function normalizeFilters(query = {}) {
  ...
  const seatFrom = Number(query.seatFrom);
  const seatTo = Number(query.seatTo);
  return {
    ...
    seatFrom: Number.isFinite(seatFrom) && seatFrom >= 0 ? seatFrom : null,
    seatTo: Number.isFinite(seatTo) && seatTo >= 0 ? seatTo : null,
...
function rowMatchesFilters(row, filters) {
  ...
  if (filters.questionSetId && Number(row.questionSetId) !== filters.questionSetId) return false;
  if (filters.status && row.status !== filters.status) return false;
  if (filters.mode && String(row.examMode || '').toUpperCase() !== filters.mode) return false;
