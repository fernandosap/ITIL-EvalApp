...
**What the e2e test server is and is not**: the e2e tests run against `tests/e2e/static-server.cjs` — a hand-written Node HTTP fixture that serves `index.html` and a 3-question mock API on `127.0.0.1:4173` (overridable via `E2E_PORT`). It is **not** the real backend; in particular the fixture's `/api/submit` always returns `{ ok: true, result: { passed: true, score: 100, total: 3, ... } }` without writing anywhere. The e2e "candidate can submit" test is therefore non-destructive **within its local fixture environment**. These Playwright tests are designed for that controlled fixture, not for pointing at production. The real submit flow is exercised by the unit tests (`tests/server-routes.test.js` exercises the production code path with a stubbed HANA) and by manual UAT. Use the dedicated post-deploy smoke for a deployed BTP/Cloud Foundry environment.
