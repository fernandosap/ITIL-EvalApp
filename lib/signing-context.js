...
function parseSigningConfig(env) {
  return {
    ...
    legacyKey: env.RESULT_SIGNING_LEGACY_KEY || ''
  };
}
...
function validateSigningConfig(config) {
  ...
  // RESULT_SIGNING_LEGACY_KEY must also be at least 32 chars.
  // Operators typically store a 64-char sha256 hex of the
  // pre-rotation derived secret; a 32-char minimum catches
  // "RESULT_SIGNING_LEGACY_KEY=x" typos. The exact format
  // (32/64-char hex) is a soft contract — we don't enforce the
  // format, only the minimum length.
  if (config.legacyKey && config.legacyKey.length < SIGNING_SECRET_MIN_LENGTH) {
    errors.push(`RESULT_SIGNING_LEGACY_KEY must be at least ${SIGNING_SECRET_MIN_LENGTH} chars (got ${config.legacyKey.length}).`);
  }
...
function createSigningContext(env, { log = () => {} } = {}) {
  ...
  return Object.freeze({
    ...
    // Legacy v1 signature: HMAC(key, JSON.stringify(payload)) only.
    // This is what historical /api/admin/results/verify-signature
    // callers used to produce. After v2 envelopes, this method is
    // kept for back-compat with the legacy endpoint contract
    // (`{ payload, signature }` pair). The result is a raw
    // signature string, NOT an envelope.
    signLegacyPayload(payload) {
      const key = keys[keyMap.current];
      return crypto
...
//
// Takes no parameters: the source of truth is always
// process.env (the CF runtime environment, which is
// effectively immutable for the life of the process). Tests
// that need a different env use createSigningContext(env,
// { log }) directly and bypass the cache via _resetForTests().
let _context = null;
let _envSnapshot = null;

const SIGNING_ENV_FINGERPRINT_KEYS = [
...
function _fingerprint() {
  const snapshot = {};
  for (const k of SIGNING_ENV_FINGERPRINT_KEYS) snapshot[k] = process.env[k] || '';
