...
async function saveSession(id, token, expiresAt) {
  return withConnection(async (conn) => {
    if (!(await tableExists(conn, 'ADMIN_SSO_SESSIONS'))) throw new Error('admin_sso_sessions_missing');
    await exec(conn,
...
async function readSession(id) {
  if (!id) return null;
  return withConnection(async (conn) => {
    if (!(await tableExists(conn, 'ADMIN_SSO_SESSIONS'))) return null;
    ...
    await exec(conn, 'UPDATE ADMIN_SSO_SESSIONS SET LAST_SEEN_AT = CURRENT_UTCTIMESTAMP WHERE SESSION_ID = ?', [String(id)]);
...
async function deleteSession(id) {
  ...
  try {
    await withConnection(async (conn) => {
      if (await tableExists(conn, 'ADMIN_SSO_SESSIONS')) await exec(conn, 'DELETE FROM ADMIN_SSO_SESSIONS WHERE SESSION_ID = ?', [String(id)]);
    });
...
async function clearSessions() {
  try {
    await withConnection(async (conn) => {
      if (await tableExists(conn, 'ADMIN_SSO_SESSIONS')) await exec(conn, 'DELETE FROM ADMIN_SSO_SESSIONS');
    });
  } catch (_e) { /* best effort */ }
}
...
async function sharedSessionMiddleware(req, _res, next) {
  try {
    ...
    if (req.path === '/api/admin/logout' && sessionId) void deleteSession(sessionId);
